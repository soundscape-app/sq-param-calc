...
class Fluctuation:
    ...
    def fmoddetection(specificLoudness, fmin=.2, fmax=64):
        ...
        phon1024b = np.reshape(phon[:, 5:235],  newshape = ( 23, phon.shape[0], 10))
        h = np.hamming(10)
        ...
        phonBm = phonB - (np.mean(phonB, 0))   
        phonBm = np.maximum(0, phonBm) 
        pbfstd = 5*(np.std(phonB, 0, ddof=1))   
        phonBm = np.minimum(phonBm, pbfstd)
        phonBf = hpfilt(phonBm) 
        pbfstd2 = np.std(phonBf, 0, ddof=1) 
        ...
        if not NP <= np.maximum(8192, 2*specificLoudness.shape[0]):
            print('Error')
            return None
        fmin = np.int(np.floor(NP*fmin/FSpec))   
        fmax = np.int(np.ceil(NP*fmax/FSpec))  
        ...
        b = np.matmul(np.linalg.pinv(np.stack((np.ones(nf), t), axis=1)), np.log(np.maximum(X[fmin-1:fmax], np.finfo(float).tiny)))
        XR = np.exp(b[1]*t+b[0])
